...
### 「couldn't load catalog」と出る／データが出ない（手編集の後）

`data/devices.json` の **JSON ミス**が原因です。文法エラーや、トップレベルが
オブジェクトでない（配列にしてしまった等）の場合、画面に
**`devices.json is not valid JSON: …（行・列）`** のような**具体的なエラー**が表示されます
（バックエンドは 503 を返し、`/api/health` は稼働継続）。表示された行を直すのが近道です。

手元での検証や、どこが悪いかの特定にはこれも便利です:

```bash
python3 -m json.tool data/devices.json   # OKなら整形表示、NGなら何行目が悪いか出る
```

直したらブラウザを再読み込み（**再起動不要で自動回復**）。だめならバックアップから戻してください。
