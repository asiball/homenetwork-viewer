...
| `/inventory` | スイッチ＆ケーブル台帳 | ポート単位の可視化＋ケーブル台帳。 | v1.1 |
| `/analysis` | リンク速度ボトルネック解析 | 既存カタログ（cat / conn / switch speed）から各リンクの上限速度を導出し、遅い順・機器別上限を表示。**オンデマンド実行**（ボタン）。 | v1.1+ |

> 補足: デザイン検討段階で「ケーブル & スイッチ・インベントリ」ビュー（ポート単位の可視化＋ケーブル台帳）も試作済み（`src/view-cables.jsx`）。実装済みの `/inventory`・`/analysis` がこれを引き継ぐ。
...
```jsonc
{
  ...
  "hist7": [ 1.0, 1.0, 0.99, 1.0, 0.92, 1.0, 1.0 ],  // 旧:手入力。#93 以降は収集サンプルから日次オンライン率を導出（このフィールドはフォールバック）
...
```
┌──────────────────────────────────────────────────────────────┐
│ ● HOMENET / NOC        ⊙radial ⑂tree   poll·5min  up 21/24  ⟳ │  header ~44px
├────────────┬───────────────────────────┬─────────────────────┤
│ DEVICE     │      TOPOLOGY MAP         │   DETAIL SUMMARY     │
│ LIST       │   (radial or tree)        │  - identity         │
...
**radial** と **tree** の2レイアウト、および両者を並べる **compare** を実装する。ヘッダー内のトグルボタンで瞬時に切り替えできること。
（当初は radial + spine の2種だったが、spine は radial と性格が近く冗長なため廃止し、実配線を示す tree に置き換えた。）

- **① radial / 放射** — gateway を中心に同心円リング状にノードを配置。`ring` (0/1/2) で半径を決める。全体把握と雰囲気重視。個人用ダッシュボード向き。
- **② tree / 配線ツリー** — switches / cables 台帳の portMap から実配線の階層を描く（gateway → switches → ports、Wi-Fi 端末は AP にぶら下げる）。固定スケールでスクロール/パン。配線点検向き。`⚡ speeds` トグルで各リンクを推定速度（cat/conn/switch speed から導出）で色分けし、ケーブルがボトルネックの箇所を強調する（§11・`/analysis` と同じ導出）。
- **③ compare / 並置** — ① radial（論理）と ② tree（物理）を上下に並べ、選択を同期して見比べる。速度オーバーレイは tree 側に効く。

### 5.3 ヘッダー

- 左: `● HOMENET / NOC` ブランドラベル + パンくず（`net 192.168.1.0/24 · iface br-lan · layout {…}`）。
- 右: **レイアウト切替トグル**（radial / tree / compare）、ポーリング状態、オンライン台数（`up 21/24`）、手動スキャンボタン（`⟳ scan`）。
...
| click | レイアウトトグル | マップを radial ⇄ tree で切り替え。 |
...
- 状態管理は React state + URL（`/?layout=tree`）のみで十分。
...
2. radial と tree の両方が実装され、ヘッダーから切り替えできる。
...
- **サブネット / VLANビュー**: VLAN/サブネットごとにグルーピングした派生レイアウト。
- **OUIによるメーカー自動判定**: MACの先頭24bitから機器メーカー名を引く。
- **履歴ページ**: 機器ごとに30日分のオンライン率・IP変動を表示。
  - 実装メモ（#93 で着手）: コレクタが到達性を追記専用の時系列（`reachability_samples`）として保存し、up/down 遷移を `reachability_events` に記録するようになった。詳細ビューの「connection · last 7 days」は、この実データから日次オンライン率を集計して表示（旧来の手入力 `hist7` はサンプルが溜まるまでのフォールバック）。`GET /api/devices/{id}/reachability?days=N` で取得でき、30日オンライン率・IP変動の本格的な履歴ページや通知基盤(#94)の土台になる。
